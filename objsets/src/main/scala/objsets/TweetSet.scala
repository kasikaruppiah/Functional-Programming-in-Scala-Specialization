...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def compareRetweet(tweet1: Tweet, tweet2: Tweet): Tweet =
      if (tweet1.retweets > tweet2.retweets) tweet1 else tweet2

    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) compareRetweet(elem, right.mostRetweeted)
    else if (right.isEmpty) compareRetweet(left.mostRetweeted, elem)
    else compareRetweet(compareRetweet(left.mostRetweeted, elem), right.mostRetweeted)
  }
